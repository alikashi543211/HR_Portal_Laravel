...
export class ViewGuard implements CanActivate {
    ...
    canActivate(next: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> | Promise<boolean> | boolean {
        ...
        /* if (state.url.includes('roles')) {
            if (permissions && permissions.roles[permissionsIndices.VIEW]) response = true;
        } else {
            if (state.url.includes('app-users')) {
                if (permissions && permissions['app-users'][permissionsIndices.VIEW]) response = true;
            }
            else {
                if (state.url.includes('admin-users')) {
                    if (permissions && permissions['admin-users'][permissionsIndices.VIEW]) response = true;
                }
            }
        } */
