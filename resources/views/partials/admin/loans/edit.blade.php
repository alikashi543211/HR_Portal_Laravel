...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            ...
            <form method="post" action="{{url(config('data.admin.loans.update'))}}">
                @csrf
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4 form-group">
                            ...
                            <select style="width: 100%" class="form-control js-example-basic" id="employeeSelect" name="user_id" required @if($loan->status != PENDING) disabled @endif>
                        ...
                        <div class="col-md-4 form-group">
                            ...
                            <input type="number" name="amount" id="amount" class="form-control" placeholder="Loan Amount" required value="{{ $loan->amount }}" @if($loan->status != PENDING) disabled @endif>
                        </div>
                        <div class="col-md-4 form-group">
                        ...
                        <div class="col-md-12 installments">
                            ...
                            <div class="row">
                                <div class="col-md-4 form-group">
                                    ...
                                    <input type="number" name="installment[]" id="{{ $key+1 }}_installment" class="form-control installment-amount" placeholder="Installment {{ $key+1 }}" required value="{{ $installment->amount }}" @if($installment->status == PAID) disabled @endif>
                                </div>
                                <div class="col-md-4 form-group">
                                    <label for="{{ $key+1 }}_installment_month">Installment {{ $key+1 }}</label>
                                    <input type="text" name="month[]" id="{{ $key+1 }}_installment_month" class="form-control datepicker" placeholder="Installment Month" required value="{{ date('F, Y',strtotime($installment->month)) }}" @if($installment->status == PAID) disabled @endif>
                                </div>
                                <div class="col-md-4">
                                    @if ($loop->first)
                                    <button class="btn btn-primary mt-4" type="button" id="add-installment" style="display: none"><i class="fas fa-plus"></i></button>
                                    @else
