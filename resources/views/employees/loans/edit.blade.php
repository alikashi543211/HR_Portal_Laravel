        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <form id="submitForm" method="POST" action="{{ route('employee.loans.update') }}">
                        @csrf
                        <input type="text" hidden name="id" value="{{ $loan->id }}">
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-4 form-group">
                                    ...
                                    <input type="number" name="amount" id="amount" value="{{ $loan->amount }}" class="form-control" placeholder="Loan Amount" required>
                                </div>
                            ...
                            <div class="row mt-5">
                                    ...
                                    @foreach ($loan->installments as $key => $installment)
                                        <div class="row">
                                            <div class="col-md-4 form-group">
                                                ...
                                                <input type="number" name="installment[]" id="{{ $key + 1 }}_installment" class="form-control installment-amount" placeholder="Installment {{ $key + 1 }}" required value="{{ $installment->amount }}" @if ($installment->status == PAID)  @endif>
                                            </div>
                                            <div class="col-md-4 form-group ">
                                                <label for="{{ $key + 1 }}_installment_month">Installment {{ $key + 1 }}</label>
                                                <input type="text" name="month[]" id="{{ $key + 1 }}_installment_month" class="form-control datepicker" placeholder="Installment Month" required value="{{ date('F, Y', strtotime($installment->month)) }}" @if ($installment->status == PAID)  @endif autocomplete="off">
                                            ...
                                            <div class="col-md-4 form-group custom-height">
                                                @if ($key === 0)
                                                    <button class="btn btn-primary  orange" type="button" id="add-installment" style=""><i class="fas fa-plus"></i></button>
                                                @else
                                                    <button class="btn btn-secondary  remove-installment orange" type="button"><i class=" fas fa-minus"></i></button>
                                                @endif
                                            </div>
                                        </div>
                                    @endforeach
                                ...
                                <div class="col-md-12 text-right ">
                                    ...
                                    <button class="btn btn-rounded btn-sm btn-outline-primary" id="submitBtn"> Submit </button>
                                </div>
                            </div>
                        </div>
                    </form>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on('click', '#add-installment', function() {
            ...
            <div class="col-md-4 form-group custom-height" style="">
                <button class="btn btn-secondary remove-installment orange" style="border-radius:5px" type="button"><i class=" fas fa-minus"></i></button>
            </div>
            </div>`;
